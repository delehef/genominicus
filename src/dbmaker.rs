...
fn parse_family(
    ...
    for l in BufReader::new(File::open(f).map_err(|e| Error::CannotOpen {
        source: e,
        filename: f.to_owned(),
    })?)
    .lines()
    ...
    ancestral_names.push(format!("ancestral-{i}"));
...
fn parse_genome(
    ...
    let species = species_regex
        .captures(
            std::path::Path::new(f)
                .file_name()
                .ok_or_else(|| Error::InvalidFilename(f.to_string()))?
                .to_str()
                .ok_or_else(|| Error::InvalidFilename(f.to_string()))?,
        )
    ...
    let mut f = File::open(f).map_err(|e| Error::CannotOpen {
        source: e,
        filename: f.to_owned(),
    })?;
...
pub fn db_from_gffs(
    ...
    let mut conn = Connection::open(db_file).map_err(|e| Error::FailedToConnect {
        source: e,
